...
export function FunctionalTeamSection({ members }: { members: TeamMemberItem[] }) {
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold text-ieee-navy border-b-2 border-ieee-red/30 pb-2 mb-8 font-times">
        Functional Team
      </h2>
      <div className="space-y-10">
        ...
        {other.length > 0 && (
          <div>
            <h3 className="text-lg font-semibold text-ieee-navy mb-4"></h3>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
              {other.map((m) => (
                <TeamCard key={m.id} member={m} />
              ))}
